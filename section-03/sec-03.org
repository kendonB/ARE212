...
This week we'll start out by reviewing some key concepts that we sped over last week. Next, we'll briefly learn how to use loops in =R=. Finally, we'll work through an example that uses many of the tools we've developed in section to demonstrate why you should be suspicious of any "$R^2$ maximizers"!
...
where $\ys = \A\y$, $\Xs_2 = \A\X_2$, and $\b_2 = (\Xsp_{2}\Xs_{2})^{-1}\Xsp_{2}\ys$.  Noting that $\A$ is both symmetric and idempotent, we can rewrite Equation (\ref{eq:rsq}) using $\A$, $\X$, and $\y$ as follows. We use that $\Xsp_{2}\Xs_{2} = \Xp_2\Ap\A\X_2 = \Xp_2\A\A\X_2 = \Xp_2\A\X_2$ and similarly that $\ysp\ys = \yp\A\y$ and $\Xsp_{2}\ys = \Xp_{2}\A\y$. We'll use this in our code below to construct $\Rsq$, constructing a general function that we can use with any arbitrary dependent vector and associated cofactor matrix:
...
Note that this code also returns $\aRsq$, the adjusted $\Rsq$. Now, what happens if we use our =randomMat()= function to add columns of random covariates to our regression? We'll use a =for= loop to calculate the $\Rsq$ and $\aRsq$ as we add additional covariates.
