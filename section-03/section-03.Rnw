...
To begin in RStudio, we need to make sure we a distribution of \LaTeX\ installed. You can
...
In the previous chunk, \texttt{readData} is the name I gave the chunk,
and \texttt{echo=TRUE} makes \texttt{knitr} display the code in the chunk.
...
Firstly,
you should note that the \texttt{xtable} package exists; this package generates
\LaTeX\ code for tables on the fly and can sometimes suffice when using \texttt{knitr}.
I seldom use \texttt{xtable}, as I like more flexibility when generating tables.
