...
We'll start section with a review of some of the common issues on the problem set. Then, we'll go over GLS and we'll use an empirical example to try out categorical dummies. Along the way, we'll try out some more advanced tools of the trade, in particular exporting to \latex and using the advanced graphing package =ggplot2=.
...
the efficiency gains from a special case of GLS, weighted least squares (WLS). We will then recreate the graphs from Figures 2.6 and 2.7, roughly, in the notes using =ggplot2= a very popular graphing package in =R=. This is part is optional, especially since it is only a very brief treatment of the
package --- there is a lot more to learn. \\

Let $x \sim U(0,2000)$ and $\epsilon \sim N(0,(x/1000)^2)$.  The underlying data generating process in (2.102) is $y_i = \alpha + x_i \beta + \epsilon$, where $\alpha = 0.5$ and $\beta = 1.5$.  The objective is to plot the simulated sampling distribution of the OLS estimator applied to $B = 10,000$ draws, each of size $n = 1000$.
After setting the seed to The Answer to the Ultimate Question of Life, the Universe, and Everything, we'll generate the sample data for one draw.
...
Now we can calculate the standard OLS parameter vector $[\ah \hspace{6pt} \bh]^{\prime}$ by noting that $\X$ is just the $x$ vector bound to a column of ones.  We will only examine $\hat{\beta}$ for this section, rather than both parameters.
...
#+RESULTS:
: [1] 1.481413 1.491242 1.503852 1.557237 1.497037 1.479635
: [1] 1.500001
