...
Let's take another crack at this. Let $\Sigma$ be our $k \times k$ given covariance matrix. We want to generate data $\X$ s.t. $V(\X) \approx \Sigma$. \\
...
then parse the returned content to list journals with certain attributes.  For this, we'll need to load the following libraries:
