...
The simplest way to deal with these three variables is to load them into a new data frame and to run either an =apply= function or a loop to produce the histogram. We'll use a =for= loop:
...
I won't  the display of the graphs in the notes, but you can run the code yourself to see three different graphs
...
#+RESULTS:
: Error: unexpected symbol in "I won"
